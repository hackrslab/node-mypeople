...
Client.prototype.sendMessage = function(buddyId, content, callback) {
  if (content != null && content.readable) {
    var r = request.post({uri: this.APIs.SEND_MESSAGE}, responseHandler(callback));
    var form = r.form();
...
Client.prototype.sendGroupMessage = function(groupId, content, callback) {
  if (content != null && content.readable) {
    var r = request.post({uri: this.APIs.GROUP_MESSAGE}, responseHandler(callback));
    var form = r.form();
...
Client.prototype.download = function(fileId, callback) {
  return new Result(request.get({uri: this.APIs.DOWNLOAD + '&fileId=' + fileId}, callback));
};
