...
Client.prototype.sendMessage = function(buddyId, content, callback) {
  if (content != null && content.readable) {
    var r = request.post({uri: this.APIs.SEND_MESSAGE, headers: headers}, responseHandler(callback));
    var form = r.form();
    form.append('buddyId', buddyId);
    form.append('attach', content, {filename: 'attachment.jpg', contentType: 'image/jpg'});
  } else {
    request.post({uri: this.APIs.SEND_MESSAGE + '&buddyId=' + buddyId
        + '&content=' + encodeURIComponent(content), headers: headers}
...
Client.prototype.getFriendInfo = function(buddyId, callback) {
  request.get({uri: this.APIs.FRIEND_INFO + '&buddyId=' + buddyId, headers: headers},
...
Client.prototype.getGroupMembers = function(groupId, callback) {
  request.get({uri: this.APIs.GROUP_MEMBERS + '&groupId=' + groupId, headers: headers},
...
Client.prototype.sendGroupMessage = function(groupId, content, callback) {
  if (content != null && content.readable) {
    var r = request.post({uri: this.APIs.GROUP_MESSAGE, headers: headers}, responseHandler(callback));
    var form = r.form();
    form.append('groupId', buddyId);
    form.append('attach', content, {filename: 'attachment.jpg', contentType: 'image/jpg'});
  } else {
    request.post({uri: this.APIs.GROUP_MESSAGE + '&groupId=' + groupId
        + '&content=' + encodeURIComponent(content), headers: headers}, responseHandler(callback));
  }
};
...
Client.prototype.exitGroup = function(groupId, callback) {
  request.get({uri: this.APIs.EXIT_GROUP + '&groupId=' + groupId, headers: headers},
    responseHandler(callback));
};
